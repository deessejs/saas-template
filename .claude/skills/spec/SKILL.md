...
4. Write    — build .claude/specs/{n}.md from PLAN_TEMPLATE.md
...
- Learning docs in `.claude/specs/` if referenced
...
cp .claude/skills/spec/PLAN_TEMPLATE.md ".claude/specs/{n}.md"
```
...
**If rejected:**
> "Spec rejected. Branch `{slug}` still exists with the spec at `.claude/specs/{n}.md`. Say the word and I'll delete it, or keep it open for later."

## §6 — Push

```bash
git add .claude/specs/{n}.md
...
- **Spec is draft** → "A draft spec exists. Review it at `.claude/specs/{n}.md` on `{slug}`, or run `/spec #{n}` again to overwrite it."
