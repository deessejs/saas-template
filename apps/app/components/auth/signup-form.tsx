...
import { signupAction } from "@/app/(unprotected)/(auth)/actions.server"
import type { SignupState } from "@/app/(unprotected)/(auth)/actions"
...
function PasswordStrength({ password }: { password: string }) {
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <p className={`text-xs ${
        strength <= 1 ? "text-destructive" : strength <= 2 ? "text-yellow-600 dark:text-yellow-400" : "text-green-600 dark:text-green-400"
      }`}>
...
export function SignupForm() {
  ...
  useEffect(() => {
    if (state && !state.ok) {
      setStrength(passwordRef.current?.value ?? "")
    }
  }, [state])
      ...
      <form action={formAction} noValidate className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1">
          ...
          {fieldErrors.name?.map((e) => (
            <p key={e} id="name-error" className="text-sm text-destructive" role="alert">
              {e}
            </p>
          ))}
        </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          <PasswordInput
            ...
            onChange={() => setStrength(passwordRef.current?.value ?? "")}
          />
          <PasswordStrength password={strength} />
          {fieldErrors.password?.map((e) => (
            <p key={e} id="password-error" className="text-sm text-destructive" role="alert">
              {e}
            </p>
          ))}
        </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          <PasswordInput
            ...
            aria-describedby={
              fieldErrors.confirmPassword?.length || confirmMismatch
          ...
          {(confirmMismatch || fieldErrors.confirmPassword?.length) && (
            <p id="confirmPassword-error" className="text-sm text-destructive" role="alert">
              {confirmMismatch ? "Passwords do not match" : fieldErrors.confirmPassword?.map((e) => e)}
            </p>
          )}
        </div>
